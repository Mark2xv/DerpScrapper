...
namespace DerpScrapper.Library
{
    public class LibraryItem : Panel
    {
        ...
        private SeriesView OpenSerieView;

        public LibraryItem(SerieInfo serie, string imageUrl = "")
        {
            Image = new PictureBox()
            {
                Size = new Size(Resources.Resources.thumb_asdf_0506.Width * 4, Resources.Resources.thumb_asdf_0506.Height * 4)
            };
        ...
        public virtual void LibraryItem_Click(object sender, EventArgs e)
        {
            if (Active)
            {
                ...
                {
                    if (OpenSerieView == null)
                    {
                        // Open Serie dialog/screen
                        SeriesView serieView = new SeriesView(this.Serie);
                        serieView.Show();
                        serieView.FormClosed += (s, ev) => { serieView.Dispose(); this.OpenSerieView = null; };
                        this.OpenSerieView = serieView;
                    }
                    else
                    {
                        OpenSerieView.BringToFront();
                        OpenSerieView.Activate();
                    }
                }
            }
            else
